...
{
    public class SetProductOfferCommandHandler : IRequestHandler<SetProductOfferCommand, ServiceResponse<bool>>
    {
        private readonly IProductService _productService;
        private readonly IMapper _mapper;

        public SetProductOfferCommandHandler(IProductService productService, IMapper mapper)
        {
        ...
        public async Task<ServiceResponse<bool>> Handle(SetProductOfferCommand request, CancellationToken cancellationToken)
        {
            var productEntity = await _productService.GetByIdAsync(request.Id);

            _mapper.Map(request, productEntity, typeof(SetProductOfferCommand), typeof(Product));
